...
class Products
{
	...
	public function addCategory($name,$desc,$vedio){
		$q = $this->con->query("SELECT * FROM categories WHERE cat_title = '$name' LIMIT 1");
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Category already exists'];
		}else{
			$q = $this->con->query("INSERT INTO categories (cat_title,description,link) VALUES ('$name','$desc','$vedio')");
			if ($q) {
	...
	public function updateCategory($post = null){
		extract($post);
		if (!empty($cat_id) && !empty($e_cat_title) && !empty($e_cat_desc) && !empty($e_cat_link)) {
			$q = $this->con->query("UPDATE categories SET cat_title = '$e_cat_title' WHERE cat_id = '$cat_id'");
			$q1 = $this->con->query("UPDATE categories SET link = '$e_cat_link' WHERE cat_id = '$cat_id'");
			$q2 = $this->con->query("UPDATE categories SET description = '$e_cat_desc' WHERE cat_id = '$cat_id'");
			if ($q && $q1 && $q2) {
			...
			return ['status'=> 303, 'message'=>'Invalid Details'];
		}
...
if (isset($_POST['add_category'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		$cat_vedio = $_POST['cat_link'];
		if (!empty($cat_title) and !empty($cat_desc) and !empty($cat_vedio)) {
			$p = new Products();
			echo json_encode($p->addCategory($cat_title,$cat_desc,$cat_vedio));
		}else{
			echo json_encode(['status'=> 303, 'message'=> 'Empty fields']);
		}
	}else{
